...
def isFunction(content, index):
    try:
        if (content[index].tokType == TYPE_ID or (content[index].tokType == TYPE_KEYW and content[index].tokContent != "typedef")) and content[index - 1].tokContent != PUNC_HASH: 
...
def isTypedef(content, index):
    if content[index].tokContent == "typedef":
        return True
    return False

def isStruct(content, index):
    if content[index].tokContent == "struct":
        return True
    return False